...
import { CubeCamera, Float, MeshReflectorMaterial, MeshTransmissionMaterial } from '@react-three/drei'
  ...
  return (
    <Canvas camera={{ position: [0, 0, 30], fov: 35, near: 1, far: 60 }} gl={{ antialias: false }}>
      ...
      <Float rotationIntensity={3} floatIntensity={3} speed={1}>
        <CubeCamera position={[0, -1, -5]} resolution={256} frames={Infinity}>
          {(texture) => (
            <mesh>
              <sphereGeometry args={[2, 32, 32]} />
              <MeshTransmissionMaterial chromaticAberration={0.2} roughness={0.2} thickness={1.6} backside envMap={texture} />
            </mesh>
          )}
        </CubeCamera>
      </Float>
